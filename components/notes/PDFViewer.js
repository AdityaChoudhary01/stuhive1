...
export default function PDFViewer({ url, fileType, title, maxPages = null }) {
  ...
  // 1. Measure container width and apply a slight horizontal gap
  useEffect(() => {
    const updateWidth = () => {
      if (containerRef.current) {
        // 🚀 ADDED GAP: Subtracting 24 leaves a 12px gap on the left and right sides
        setContainerWidth(containerRef.current.clientWidth - 24); 
      }
    };
    updateWidth();
    
    // Slight delay on mount to ensure layout is painted
  ...
  return (
    ...
    >
        ...
        >
          {/* 🚀 ADDED GAP: pt-4 gives a slight gap at the very top of the PDF */}
          <div className="flex flex-col items-center min-w-max pb-24 pt-4">
            ...
            >
               {Array.from(new Array(pagesToRender || 0), (el, index) => (
                  <div 
                    key={`page_${index + 1}`} 
                    // 🚀 ADDED GAP: mb-4 creates distinct spacing between pages, shadow-xl makes them pop
        ...
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-50">
          <div className="flex items-center gap-1 bg-background/95 backdrop-blur-xl border border-border px-2 py-1.5 rounded-full shadow-2xl">
             ...
             <div className="text-[11px] md:text-xs font-bold text-foreground w-12 text-center select-none">
